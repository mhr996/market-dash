...
interface DeliveryStatisticsFilterProps {
    shops: FilterOption[];
...
const DeliveryStatisticsFilter: React.FC<DeliveryStatisticsFilterProps> = ({ shops, drivers, onFilterChange, currentFilters, isLoading = false }) => {
    ...
    const filteredDrivers = drivers.filter((driver) => driver.name.toLowerCase().includes(searchTerms.drivers.toLowerCase()));
    ...
    const selectedDrivers = drivers.filter((driver) => currentFilters.drivers.includes(driver.id));
    ...
    // Toggle driver selection in pending filters
    const toggleDriver = (driverId: number) => {
        const newDrivers = pendingFilters.drivers.includes(driverId) ? pendingFilters.drivers.filter((id: number) => id !== driverId) : [...pendingFilters.drivers, driverId];
        updatePendingFilters({ drivers: newDrivers });
    };
    ...
    // Remove specific driver from applied filters
    const removeDriver = (driverId: number) => {
        const newFilters = {
            ...currentFilters,
            drivers: currentFilters.drivers.filter((id: number) => id !== driverId),
        };
        onFilterChange(newFilters);
    };
    ...
    const hasActiveFilters = currentFilters.shops.length > 0 || currentFilters.drivers.length > 0 || currentFilters.timeRange !== 'all';
    ...
    const getActiveFilterCount = () => {
        return currentFilters.shops.length + currentFilters.drivers.length + (currentFilters.timeRange !== 'all' ? 1 : 0);
    };
                ...
                <div className="mb-4 p-4 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm">
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {selectedDrivers.map((driver) => (
                            ...
                            >
                                {driver.logo_url && <img src={driver.logo_url} alt={driver.name} className="h-4 w-4 rounded-full object-cover" />}
                                <span className="max-w-[100px] truncate font-medium">{driver.name}</span>
                                <button
                                    type="button"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300">Filter by Shops</label>
                                    <div className="space-y-3">
                                ...
                                {/* Time Range Filter */}
                                <div className="space-y-3">
                                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300">Time Range</label>
                                    <select
                            ...
                            <div className="space-y-6">
                                {/* Driver Filter */}
                                <div className="space-y-3">
                                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300">Filter by Drivers</label>
                                    <div className="space-y-3">
                                        <div className="relative">
                                            <input
                                                type="text"
                                                ...
                                                onChange={(e) => setSearchTerms((prev) => ({ ...prev, drivers: e.target.value }))}
                                            />
                                            <IconSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                                        </div>
                                        <div className="max-h-56 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900">
                                            {filteredDrivers.length > 0 ? (
                                                filteredDrivers.map((driver) => (
                                                    ...
                                                    >
                                                        <input
                                                            type="checkbox"
                                                            checked={pendingFilters.drivers.includes(driver.id)}
                                                            onChange={() => toggleDriver(driver.id)}
                                                            className="rounded border-gray-300 text-primary focus:ring-primary w-4 h-4"
                                                        />
                                                        {driver.logo_url && (
                                                            <img src={driver.logo_url} alt={driver.name} className="w-8 h-8 rounded-full object-cover ring-2 ring-gray-200 dark:ring-gray-600" />
                                                        )}
...
export default DeliveryStatisticsFilter;

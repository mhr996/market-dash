...
const BrandsList = () => {
    ...
    useEffect(() => {
        const fetchBrands = async () => {
            try {
                const { data, error } = await supabase.from('categories_brands').select('*, shops(id, shop_name)').order('created_at', { ascending: false });
                                ...
                                render: ({ name, image_url }) => (
                                    <div className="flex items-center font-semibold">
                                        <div className="w-max rounded-full ltr:mr-2 rtl:ml-2">
                                            {image_url ? (
                                                <img className="h-8 w-8 rounded-md object-cover" src={image_url} alt={name} />
                                            ) : (

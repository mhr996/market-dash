...
const InvoicesList = () => {
    ...
    useEffect(() => {
        const fetchInvoices = async () => {
            try {
                // Fetch completed orders for invoices with shipment focus
    ...
    useEffect(() => {
        setInitialRecords(
            items.filter((item) => {
                ...
                return item.invoice_number.toLowerCase().includes(searchTerm) || item.shop_name.toLowerCase().includes(searchTerm) || item.shipping_method.toLowerCase().includes(searchTerm);
            }),
        );
    }, [items, search]);

    useEffect(() => {
        const sorted = sortBy(initialRecords, sortStatus.columnAccessor as keyof Invoice);
    ...
    const handleDownloadPDF = (orderId: number) => {
        ...
        router.push(`/orders/preview/${orderId}?type=invoice`);
    };
            ...
            <div className="invoice-table w-full max-w-none">
                <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center">
                    <div className="flex items-center gap-2">
                        <h5 className="text-lg font-semibold dark:text-white-light">Invoices</h5>
                        <p className="text-gray-500 dark:text-gray-400">Completed orders invoices with shipment info</p>
                    </div>
                ...
                <div className="datatables pagination-padding relative w-full max-w-none">
                    <DataTable
                        ...
                        records={records}
                        onRowClick={(record) => {
                            router.push(`/orders/preview/${record.order_id}?type=invoice`);
                        }}
                        columns={[
                            {
                            ...
                            },
                            {
                                ...
                                sortable: true,
                                render: ({ shipping_method }) => <span className="badge bg-info text-white">{shipping_method}</span>,
                            },
                            {
                                ...
                                render: ({ shipping_address }) => {
                                    const address = typeof shipping_address === 'string' ? shipping_address : (shipping_address as any)?.address || 'No address provided';
                                    return <span className="text-gray-600 text-sm">{address.slice(0, 30)}...</span>;
                                },
                            },
                            {
                                accessor: 'created_at',

...
const AddShopCategoryPage = () => {
    ...
    const uploadImageToCategory = async (categoryId: number, file: File): Promise<string> => {
        ...
        const { error: uploadError } = await supabase.storage.from('shop-categories').upload(fileName, file, {
            cacheControl: '3600',
            upsert: true,
        });
        ...
        const {
            data: { publicUrl },
        } = supabase.storage.from('shop-categories').getPublicUrl(fileName);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (imageFile && categoryData) {
                ...
                const { error: updateError } = await supabase.from('categories_shop').update({ image_url: imageUrl }).eq('id', categoryData.id);
    ...
    return (
        <div className="container mx-auto p-6 w-full max-w-none">
            ...
            <div className="rounded-md border border-[#ebedf2] bg-white p-4 dark:border-[#191e3a] dark:bg-black">
                <h6 className="mb-5 text-lg font-bold">{t('add_new_shop_category')}</h6>
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            <label htmlFor="description" className="block text-sm font-bold text-gray-700 dark:text-white">
                                {t('description')}
                            </label>
                            <textarea
                                id="description"
...
export default AddShopCategoryPage;

...
const PreviewShopSubCategory = () => {
    ...
    useEffect(() => {
        const fetchSubCategory = async () => {
            try {
                const { data, error } = await supabase.from('categories_sub_shop').select('*, categories_shop(*)').eq('id', subCategoryId).single();
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const { data: files } = await supabase.storage.from('shop-subcategory-images').list(folderPath);

            if (files && files.length > 0) {
                const filesToDelete = files.map((file) => `${folderPath}/${file.name}`);
                await supabase.storage.from('shop-subcategory-images').remove(filesToDelete);
            }

            // Then delete the subcategory from database
            const { error } = await supabase.from('categories_sub_shop').delete().eq('id', subCategory.id);
            if (error) throw error;
        ...
        return (
            <div className="panel">
                <div className="text-center py-8">
                    ...
                    <Link href="/shops/categories/subcategories" className="btn btn-primary mt-4">
            ...
            <div className="mb-5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h5 className="text-lg font-semibold dark:text-white-light">Shop Subcategory Preview</h5>
                    </div>
                    <div className="flex items-center gap-2">
                        <Link href={`/shops/categories/subcategories/edit/${subCategory.id}`} className="btn btn-primary gap-2">
                            <IconEdit />
                            Edit
                        </Link>
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-700 dark:text-gray-300">{subCategory.categories_shop?.title || 'N/A'}</p>
                        </div>
                        <div>
                ...
                <div>
                    ...
                    <div className="flex justify-center">
                        {subCategory.image_url ? (
                            <img src={subCategory.image_url} alt={subCategory.title} className="w-64 h-64 object-cover rounded-lg border border-gray-200 dark:border-gray-700" />
                        ) : (
...
export default PreviewShopSubCategory;

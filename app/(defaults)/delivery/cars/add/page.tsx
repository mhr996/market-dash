...
const AddDeliveryCarPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error } = await supabase.from('delivery_cars').insert([insertPayload]);
            if (error) throw error;

            setAlert({ visible: true, message: t('car_added_successfully'), type: 'success' });
            router.push('/delivery/cars');
        } catch (error: any) {
            setAlert({ visible: true, message: error.message || t('error_adding_car'), type: 'danger' });
            setLoading(false);
        }
    };
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex justify-end gap-4">
                    ...
                    <button type="submit" disabled={loading} className="btn btn-primary">
                        {loading ? t('submitting') : t('add_car')}
                    </button>
                </div>
            </form>
        </div>
